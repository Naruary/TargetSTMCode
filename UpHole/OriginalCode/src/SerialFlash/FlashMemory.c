...
{
	if (pageNumber < Serial_Flash_Chip.Max_pages_available)
	{
		return 1;
	}
	return 0;
...
U_INT32 Serflash_find_next_event_slot(U_INT32 event_block_size)
{
	...
	while (loopy < Serial_Flash_Chip.EVENTS_pages_available)
	{
		...
		if ((Serflash_page_data[0] == 0xFF) && (Serflash_page_data[1] == 0xFF))
		{
			return target_page;
		}
		loopy++;
...
U_BYTE Serflash_read_NV_Block(void)
{
	...
	if (sizeof(NVRAM_data) > Serial_Flash_Chip.page_size)
		return 0;
...
U_BYTE Serflash_check_NV_Block(void)
{
	...
	if (howmuch_data > Serial_Flash_Chip.page_size)
	{
		return 0;
	}
	// we cannot do this too often, we will wear out the flash
	if (ElapsedTimeLowRes(g_tFlashIdleTimer) < (TEN_SECOND))
	{
		return 0;
	}
...
U_BYTE Serflash_read_Borehole_Block(void)
{
	...
	if (sizeof(boreholeStatistics) > Serial_Flash_Chip.page_size)
	{
		return 0;
	}
	// go get all of our data from flash
...
U_BYTE Serflash_read_Newhole_Block(void)
{
	...
	if (sizeof(newHole_tracker) > Serial_Flash_Chip.page_size)
	{
		return 0;
	}
	// go get all of our data from flash
...
void Serflash_read_DID_data(void)
{
	...
	{
		if ((Serial_Flash_Chip.manufacturer_code == FLASH_DATA[loopy].mfg_code) && (Serial_Flash_Chip.memory_type == FLASH_DATA[loopy].mem_type)
	...
	Serial_Flash_Chip.page_size = FLASH_DATA[Serial_Flash_Chip.part_index].page_size;
	Serial_Flash_Chip.Max_pages_available = FLASH_DATA[Serial_Flash_Chip.part_index].num_pages;
	Serial_Flash_Chip.NV_test_page = FLASH_DATA[Serial_Flash_Chip.part_index].startof_page1;
	Serial_Flash_Chip.NV_param_start_page = FLASH_DATA[Serial_Flash_Chip.part_index].startof_page2;
	Serial_Flash_Chip.Borehole_start_page = FLASH_DATA[Serial_Flash_Chip.part_index].startof_page3;
	Serial_Flash_Chip.Newhole_start_page = FLASH_DATA[Serial_Flash_Chip.part_index].startof_page4;
	Serial_Flash_Chip.EVENTS_start_page = FLASH_DATA[Serial_Flash_Chip.part_index].startof_page5;
	Serial_Flash_Chip.EVENTS_pages_available = FLASH_DATA[Serial_Flash_Chip.part_index].num_pages;
...
void Check_NV_data_boundaries(void)
{
	...
	if ((NVRAM_data.fKeyBeeperEnable < NVRAM_min.fKeyBeeperEnable) || (NVRAM_data.fKeyBeeperEnable > NVRAM_max.fKeyBeeperEnable))
		NVRAM_data.fKeyBeeperEnable = NVRAM_defaults.fKeyBeeperEnable;
	if ((NVRAM_data.fCheckShot < NVRAM_min.fCheckShot) || (NVRAM_data.fCheckShot > NVRAM_max.fCheckShot))
		NVRAM_data.fCheckShot = NVRAM_defaults.fCheckShot;
	if ((NVRAM_data.nBacklightOnTime_sec < NVRAM_min.nBacklightOnTime_sec) || (NVRAM_data.nBacklightOnTime_sec > NVRAM_max.nBacklightOnTime_sec))
		NVRAM_data.nBacklightOnTime_sec = NVRAM_defaults.nBacklightOnTime_sec;
	if ((NVRAM_data.nLCDOnTime_sec < NVRAM_min.nLCDOnTime_sec) || (NVRAM_data.nLCDOnTime_sec > NVRAM_max.nLCDOnTime_sec))
		NVRAM_data.nLCDOnTime_sec = NVRAM_defaults.nLCDOnTime_sec;
	...
	if ((NVRAM_data.nDeclination < NVRAM_min.nDeclination) || (NVRAM_data.nDeclination > NVRAM_max.nDeclination))
		NVRAM_data.nDeclination = NVRAM_defaults.nDeclination;
	if ((NVRAM_data.nDesiredAzimuth < NVRAM_min.nDesiredAzimuth) || (NVRAM_data.nDesiredAzimuth > NVRAM_max.nDesiredAzimuth))
	{
		NVRAM_data.nDesiredAzimuth = 0.0;
	}
	if ((NVRAM_data.nCheckPollTime_sec < NVRAM_min.nCheckPollTime_sec) || (NVRAM_data.nCheckPollTime_sec > NVRAM_max.nCheckPollTime_sec))
		NVRAM_data.nCheckPollTime_sec = NVRAM_defaults.nCheckPollTime_sec;
	if ((NVRAM_data.loggingState < NVRAM_min.loggingState) || (NVRAM_data.loggingState > NVRAM_max.loggingState))
		NVRAM_data.loggingState = NVRAM_defaults.loggingState;
}
