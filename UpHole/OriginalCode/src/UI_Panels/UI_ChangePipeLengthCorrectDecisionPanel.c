...
static void ChangePipeLengthCorrect_Decision_Paint(TAB_ENTRY * tab)
{
	...
	TabWindowPaint(tab);
	//ZD September 22 2024 needs to display float for pipe length
	float PipeLengthStringVariable = GetNewPipeLength() / 100.0;
	snprintf(stringValue, 40, "%s %5.2f%s", "Pipe Length =", PipeLengthStringVariable, ", Are you sure?");
